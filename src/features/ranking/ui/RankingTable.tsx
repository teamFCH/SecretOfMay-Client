  ...
  if (rankings.length === 0) {
    return (
      <div className="py-12 text-center">
        <p className="font-mono text-[13px] tracking-[0.15em] text-parchment-muted">
          아직 기록된 복원 요원이 없습니다
        </p>
        <p className="mt-2 font-mono text-[11px] tracking-widest text-parchment-muted/50">
  ...
  return (
    <div className="overflow-hidden border border-wire/40">
      <table className="w-full border-collapse table-fixed">
        ...
        <thead>
          <tr className="border-b-2 border-wire/60">
            <HeaderCell className="w-10 sm:w-12">순위</HeaderCell>
            <HeaderCell>요원명</HeaderCell>
            <HeaderCell className="w-20 text-right">기록</HeaderCell>
            <HeaderCell className="hidden sm:table-cell w-24 text-right">날짜</HeaderCell>
          </tr>
        </thead>
        <tbody>
          {rankings.map((entry, index) => {
            ...
            return (
              ...
              >
                ...
                <BodyCell
                  className={cn(
                    "truncate font-serif text-[13px] sm:text-[14px]",
                    colorClassName,
                ...
                >
                  {entry.playerName}
                  {isCurrentPlayer && (
                    <span className="ml-1.5 font-mono text-[10px] tracking-widest text-parchment-muted not-italic">
                      ← 나
                    </span>
                  )}
                </BodyCell>
                ...
                <BodyCell className={cn("hidden sm:table-cell text-right tabular-nums text-[10px]", colorClassName)}>
      ...
      <div className="mt-1 border-t border-wire/40 px-3 pt-2">
        <p className="text-right font-mono text-[11px] tracking-widest text-parchment-muted/50">
...
function HeaderCell({ children, className }: { children: ReactNode; className?: string }) {
  return (
    <th
      scope="col"
      className={cn(
        "px-2 pb-2 sm:px-3 sm:pb-2.5 text-left font-mono text-[11px] tracking-[0.2em] text-parchment-muted",
        className,
      )}
    >
      {children}
...
function BodyCell({ children, className }: { children: ReactNode; className?: string }) {
  return <td className={cn("px-2 py-2 sm:px-3 sm:py-2.5 font-mono text-[12px] sm:text-[13px]", className)}>{children}</td>;
}
