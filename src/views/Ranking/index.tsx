    ...
    <ArchivePage panelClassName="max-w-[580px]" contentClassName="flex flex-col gap-6">
      <div className="flex flex-col gap-1">
        ...
        <div className="flex items-start justify-between gap-4 border-t border-wire/60 pt-3">
          <div>
            <h1 className="font-serif text-[24px] font-black tracking-[0.15em] text-parchment">
              복원 완료자 명단
            </h1>
            <p className="mt-1 font-mono text-[10px] tracking-[0.12em] text-parchment-muted/60">
              문서번호 : {RANKING_DOCUMENT_ID}
            </p>
          </div>

          <div
            className="shrink-0 rotate-[4deg] border border-blood/50 px-3 py-1.5 font-mono text-[11px] tracking-[0.25em] text-blood/60"
            aria-hidden="true"
          >
            열람
          </div>
      ...
      <div className="relative border-t border-wire/50">
        <span className="absolute left-1/2 -translate-x-1/2 -translate-y-1/2 bg-paper px-3 font-mono text-[9px] text-parchment-muted/50">
          · · ·
        </span>
      </div>

      <p className="border-l border-wire/40 pl-3 font-mono text-[9px] leading-[1.8] text-parchment-muted/70">
